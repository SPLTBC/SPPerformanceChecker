codeunit 56018 "SP Perf. Checker Post Item Jnl" implements "BCPT Test Param. Provider"
{
    SingleInstance = true;
    ...
    begin
        exit(CopyStr(ItemNoParamLbl + '=', 1, 1000));
    end;

    procedure ValidateParameters(Parameters: Text[1000])
    begin
        if StrPos(Parameters, ItemNoParamLbl) > 0 then begin
            Parameters := DelStr(Parameters, 1, StrLen(ItemNoParamLbl + '='));
            if Evaluate(ItemNo, Parameters) then
                exit;
        end;
